...
public class StudentHomeController extends BaseController {
    ...
    public void initialize() {
        ...
        if (user != null) {
            welcomeLabel.setText("欢迎你，" + user.getName() + "同学");
        }
        ...
        // "选课"按钮
        courseActionBtn.setOnAction(e -> {
            NavigationManager.navigateTo("course-selection-view.fxml", null);
        });

        // "查看更多"公告
        moreAnnouncementsBtn.setOnAction(e -> {
            NavigationManager.navigateTo("list-page-view.fxml", (ListPageController controller) -> {
                controller.setPageType(ListPageController.PageType.ANNOUNCEMENT, null, "STUDENT");
            });
        });
    ...
    private void loadCourses(int page) {
        ...
        try {
            PageResult<Course> result = ApiClient.getMyCourses(page, 10);
            if (result != null) {
